# Obtained from: https://github.com/Guanys-dar/MGDN
# Modifications:
# Added the block class
#
#-*- coding:utf-8 -*-
import math
import time
...
class Block(nn.Module):
    def __init__(self,in_channels):
        super().__init__()
        self.encode = DeepMuGIF(in_channels)
        self.decode = HDRTransformer_backbone()
        
    def forward(self,inputs):
        sam,pl_source = inputs
        sam,pl_source = self.encode(sam,pl_source)
        decode = self.decode(sam + pl_source)
        return decode
